// Generated by ReScript, PLEASE EDIT WITH CARE

import * as Curry from "rescript/lib/es6/curry.js";
import * as Command from "./Shared/Command.bs.js";
import * as Electron from "./Interop/Electron.bs.js";
import * as Belt_Option from "rescript/lib/es6/belt_Option.js";
import * as Caml_splice_call from "rescript/lib/es6/caml_splice_call.js";

function onCommand(listener) {
  return Electron.IpcMain.on("main", (function ($$event, args) {
                return Belt_Option.map(Command.decodeCommand(args), (function (cmd) {
                              return Curry._2(listener, $$event, cmd);
                            }));
              }));
}

function sendCommand(command, webContents) {
  Caml_splice_call.spliceObjApply(webContents, "send", [Command.encodeCommand(command)]);
  
}

export {
  onCommand ,
  sendCommand ,
  
}
/* Electron Not a pure module */
